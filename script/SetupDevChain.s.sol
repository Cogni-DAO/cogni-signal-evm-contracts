...
contract SetupDevChain is Script {
    ...
    function _printEnvironmentVariables(DeploymentResult memory result) internal view {
        ...
        console2.log("==============================================");
        console2.log("## Chain Configuration - Copy/paste to cogni-git-admin .env file");
        console2.log("");
        console2.log("WALLET_PRIVATE_KEY=", vm.envString("WALLET_PRIVATE_KEY"));
        console2.log("EVM_RPC_URL=", vm.envString("EVM_RPC_URL"));
        console2.log("SIGNAL_CONTRACT=", vm.toString(result.cogniSignal));
        console2.log("DAO_ADDRESS=", vm.toString(result.dao));
        console2.log("CHAIN_ID=", vm.toString(result.chainId));
        
        // Print admin plugin address if available (for Aragon OSx)
        if (result.adminPlugin != address(0)) {
            console2.log("ARAGON_ADMIN_PLUGIN_CONTRACT=", vm.toString(result.adminPlugin));
        } else {
            console2.log("# ARAGON_ADMIN_PLUGIN_CONTRACT=  # Not applicable for ", result.govProviderType);
        }
        console2.log("");
        console2.log("## Optional Extras");
        console2.log("GOVERNANCE_TOKEN=", vm.toString(result.token));
        console2.log("TOKEN_NAME=", result.tokenName);
        console2.log("TOKEN_SYMBOL=", result.tokenSymbol);
        console2.log("GOV_PROVIDER_TYPE=", result.govProviderType);
        console2.log("DEPLOYER_ADDRESS=", vm.toString(result.deployer));
        console2.log("");
    }
    ...
    function _saveEnvironmentVariablesToFile(DeploymentResult memory result) internal {
        ...
        string memory envContent = string.concat(
            ...
            "WALLET_PRIVATE_KEY=", vm.envString("WALLET_PRIVATE_KEY"), "\n",
